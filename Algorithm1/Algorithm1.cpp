...
int crossWeightOf(Edge e, state s){
	bool isCrossEdge = false;
	string u = e.endpoints[0];
	string v = e.endpoints[1];
	for(auto partition:s)
		if(partition.statePhi.find(u)!=partition.statePhi.end() && partition.statePhi.find(v)==partition.statePhi.end()) //it contains u but not v
		{
			isCrossEdge = true;
			break;
		}
	return isCrossEdge * e.weight;
}

int computeDPIntroduceEdge(Bag* bag, state s){
	return computeDP(bag->children[0], s) + crossWeightOf(bag->introducedEdge, s);
}
